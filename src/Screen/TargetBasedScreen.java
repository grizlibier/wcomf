...
public abstract class TargetBasedScreen implements Screen {
    ...
    public void displayOutput(AsciiPanel terminal) {
        for (Point p : new Line(sx, sy, sx + x, sy + y)){
            if (p.x < 0 || p.x >= width || p.y < 0 || p.y >= height - 16) {
            	continue;
            }
            
            terminal.write('X', p.x, p.y, AsciiPanel.brightYellow);
        }
        
        terminal.clear(' ', 0, height - 15, width, 1);
    ...
    public Screen respondToUserInput(KeyEvent key) {
        ...
        case KeyEvent.VK_ENTER: selectWorldCoordinate(player.x + x, player.y + y, sx + x, sy + y); player.world.update(); return null;
